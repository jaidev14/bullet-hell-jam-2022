...
namespace TopDownCharacter2D.Stats
{
    ...
    public class CharacterStatsHandler : MonoBehaviour
    {
        ...
        {
            AttackConfig _attackConfig = null;
            if (baseStats.attackConfig != null)
            {
                _attackConfig = Instantiate(baseStats.attackConfig);
            }
            DashConfig _dashConfig = null;
            if (baseStats.dashConfig != null)
            {
                _dashConfig = Instantiate(baseStats.dashConfig);
            }

            CurrentStats = new CharacterStats {attackConfig = _attackConfig, dashConfig = _dashConfig};
